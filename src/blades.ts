...
export const GREEN_BLADE_FAR = 16;
/** Max instanced blades on a green. */
export const GREEN_BLADE_MAX = 2800;
...
export function greenBladeLod(camDistYards: number): { visible: boolean; opacity: number; density: number } {
  ...
  if (camDistYards <= GREEN_BLADE_NEAR) return { visible: true, opacity: 0.92, density: 1 };
  const t = 1 - (camDistYards - GREEN_BLADE_NEAR) / (GREEN_BLADE_FAR - GREEN_BLADE_NEAR);
  return { visible: true, opacity: 0.2 + t * 0.72, density: t * t };
}
...
function makeBladeCard(): THREE.DataTexture {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      const a = edge > 0.08 ? Math.min(1, edge * 1.15) * (0.72 + n * 0.28) : 0;
      const g = 70 + vein * 50 + n * 36;
      const r = 28 + n * 22;
      const b = 18 + n * 10;
      const i = (y * w + x) * 4;
      data[i] = r;
...
export function createBladeMaterial(): THREE.MeshStandardMaterial {
  ...
  return new THREE.MeshStandardMaterial({
    ...
    metalness: 0,
    depthWrite: false,
    emissive: new THREE.Color(0x1e4a1c),
...
export function buildGreenBladeField(hole: Hole, mat: THREE.MeshStandardMaterial): THREE.InstancedMesh {
  ...
  for (let i = 0; i < tries && mats.length < GREEN_BLADE_MAX; i++) {
    ...
    if (turfBand(hole, x, z) !== "green" && turfBand(hole, x, z) !== "collar") continue;
    const y = groundHeight(hole, x, z) + 0.012;
    const h = 0.018 + hashNoise(i, 7) * 0.016;
    const w = 0.006 + hashNoise(i, 9) * 0.005;
    dummy.position.set(x, y + h * 0.5, z);
    dummy.rotation.set(0, hashNoise(i, 3) * Math.PI * 2, (hashNoise(i, 5) - 0.5) * 0.18);
